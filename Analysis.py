...
# create doc2vec input takes a path and label as an argument and puts together all file to one file for every class
    ...
    def test_doc2vec(self,train_path_d2v, test_path_d2v, input_model, percentage,n, size,output_file_name):
       ...
       # print(test_array.shape)
        ...
        start = time.time()
        clf_LR.fit(train_arrays, train_labels)
        myEval_nFoldCV_LR = estimation.evaluate_nFoldCV(test_array, test_labels, clf_LR, n)
        end = time.time()
    ...
    def test_tfidf(self,train_path_tfidf,test_path_tfidf,percentage,n,output_file_name,use_idf):
        ...
        start = time.time()
        clf_tfidf.fit(dataset, classLabels)
        ...
        myEval_nFoldCV_SVM = estimation_tfidf.evaluate_nFoldCV(dataset_test, classLabels_test, clf_tfidf, n)
        end = time.time()
        time_ = (end - start)
        ...
        classLabels_test = self.classLabels
        dataset_test = Tfidf_Vectorizer_test.dataset
        myEval_nFoldCV_LR = estimation_tfidf.evaluate_nFoldCV(dataset_test, classLabels_test, clf_LR, n)
        end = time.time()
